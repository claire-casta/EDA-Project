...
class DataTransform:
    """Various methods that transform the data within the dataframe."""
...
class DataframeInfo:
    """Various methods that give information about the dataframe."""
    def __init__(self, df):
        self.df = df
...
class DataFrameTransform:
     """Transforms the data using various methods"""                               
     def __init__(self, df):
        self.df = df
     ...
     def impute_mode(self, column_name):
         """Replaces null value with mode value"""
     ...
     def impute_term(self, value_column, loan_amount, int_rate, instalment):
         """Imputes a loan term into the value_columm (which in this project is loan_term) using a calculation that takes values from the loan amount, interest 
     ...
     def apply_yeo_johnson(self, column_name):
        """Transforms the data using the Yeo Johnson method"""                               
     ...
     def remove_outliers_IQR(self, column_name):
        """Removes outliers below the 25th centile and above the 75th centile for a named column, using the interquartile range."""
